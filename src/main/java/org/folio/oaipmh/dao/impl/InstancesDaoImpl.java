...
public class InstancesDaoImpl implements InstancesDao {
    ...
    return getQueryExecutorReader(tenantId).transaction(queryExecutor ->
      ...
      .where(REQUEST_METADATA_LB.REQUEST_ID.eq(UUID.fromString(requestId))))
          ...
          .map(optionalRequestMetadata -> {
            ...
            throw new NotFoundException(String.format(REQUEST_METADATA_WITH_ID_DOES_NOT_EXIST,
                requestId));
          }));
  }
  ...
  public Future<RequestMetadataCollection> getRequestMetadataCollection(int offset,
    ...
    return getQueryExecutorReader(tenantId)
        .transaction(queryExecutor ->
            queryExecutor.query(dslContext -> dslContext.selectCount().from(REQUEST_METADATA_LB)))
        .compose(recordsCount -> getQueryExecutorReader(tenantId)
            .transaction(queryExecutor ->
                queryExecutor
                    .query(dslContext ->
                        dslContext
                            ...
                            .offset(offset)
                            .limit(limit))
                    .map(collection -> queryResultToRequestMetadataCollection(collection,
        ...
        .transaction(queryExecutor ->
            queryExecutor.query(dslContext ->
                dslContext.selectCount()
                    .from(FAILED_TO_SAVE_INSTANCES_IDS)
                    .where(FAILED_TO_SAVE_INSTANCES_IDS.REQUEST_ID
                        .eq(UUID.fromString(requestId)))))
        .compose(recordsCount ->
            getQueryExecutorReader(tenantId)
                .transaction(queryExecutor ->
                    queryExecutor
                        .query(dslContext ->
                            dslContext
                                .selectFrom(FAILED_TO_SAVE_INSTANCES_IDS)
                                .where(FAILED_TO_SAVE_INSTANCES_IDS.REQUEST_ID
                                    .eq(UUID.fromString(requestId)))
                                .offset(offset)
                                .limit(limit))
                        .map(collection -> queryResultToUuidCollection(collection,
                            recordsCount.get(0, int.class)))));
  }
    ...
    return getQueryExecutorReader(tenantId)
        .transaction(queryExecutor ->
            queryExecutor.query(dslContext ->
                dslContext
                    .selectCount()
                    .from(SKIPPED_INSTANCES_IDS)
                    .where(SKIPPED_INSTANCES_IDS.REQUEST_ID.eq(UUID.fromString(requestId)))))
        .compose(recordsCount ->
            getQueryExecutorReader(tenantId)
                .transaction(queryExecutor ->
                    queryExecutor
                        .query(dslContext -> dslContext.selectFrom(SKIPPED_INSTANCES_IDS)
                            .where(SKIPPED_INSTANCES_IDS.REQUEST_ID.eq(UUID.fromString(requestId)))
                            .offset(offset)
                            .limit(limit))
                        .map(collection -> queryResultToUuidCollection(collection,
            ...
            getQueryExecutorReader(tenantId)
                .transaction(queryExecutor ->
                    queryExecutor
                        .query(dslContext ->
                            dslContext
                                .selectFrom(FAILED_INSTANCES_IDS)
                                .where(FAILED_INSTANCES_IDS.REQUEST_ID
                                    .eq(UUID.fromString(requestId)))
                                .offset(offset)
                                .limit(limit))
                        .map(collection -> queryResultToUuidCollection(collection,
  ...
  @Override
  public Future<UuidCollection> getSuppressedInstancesIdsCollection(String requestId, int offset,
      int limit, String tenantId) {
    return getQueryExecutorReader(tenantId)
        .transaction(queryExecutor ->
            queryExecutor.query(dslContext ->
                dslContext.selectCount().from(SUPPRESSED_FROM_DISCOVERY_INSTANCES_IDS)
                    .where(SUPPRESSED_FROM_DISCOVERY_INSTANCES_IDS.REQUEST_ID
                        .eq(UUID.fromString(requestId)))))
        .compose(recordsCount ->
            getQueryExecutorReader(tenantId)
                .transaction(queryExecutor ->
                    queryExecutor.query(dslContext ->
                        dslContext.selectFrom(SUPPRESSED_FROM_DISCOVERY_INSTANCES_IDS)
                            .where(SUPPRESSED_FROM_DISCOVERY_INSTANCES_IDS.REQUEST_ID
                                .eq(UUID.fromString(requestId)))
                            .offset(offset)
                            .limit(limit))
  ...
  public Future<RequestMetadataLb> saveRequestMetadata(RequestMetadataLb requestMetadata,
    ...
    if (Objects.isNull(uuid) || StringUtils.isEmpty(uuid.toString())) {
      return Future.failedFuture(new IllegalStateException("Cannot save request metadata, request "
          + "metadata entity must contain requestId"));
    }
    ...
    if (Objects.isNull(startedDate) || StringUtils.isEmpty(startedDate.toString())) {
      return Future.failedFuture(new IllegalStateException(
          "Cannot save request metadata, request metadata entity must contain startedDate"));
    }
    return getQueryExecutor(tenantId)
        .transaction(queryExecutor -> queryExecutor
  ...
  public Future<RequestMetadataLb> updateRequestUpdatedDateAndStatistics(String requestId,
    ...
    return getQueryExecutor(tenantId).transaction(queryExecutor -> {
      ...
      var saveFailedToSaveInstancesIds = holder.getFailedToSaveInstancesIds().isEmpty()
          ? Future.succeededFuture()
          : queryExecutor.execute(dslContext -> {
            InsertValuesStep3<FailedToSaveInstancesIdsRecord, UUID, UUID, UUID> insertValues =
                dslContext.insertInto(FAILED_TO_SAVE_INSTANCES_IDS,
                  FAILED_TO_SAVE_INSTANCES_IDS.ID, FAILED_TO_SAVE_INSTANCES_IDS.REQUEST_ID,
                  FAILED_TO_SAVE_INSTANCES_IDS.INSTANCE_ID);
            holder.getFailedToSaveInstancesIds()
                .forEach(id -> insertValues.values(UUID.randomUUID(),
                    UUID.fromString(requestId), UUID.fromString(id)));
            return insertValues;
          }).map(rows -> null);

      var saveFailedInstancesIds = holder.getFailedInstancesIds().isEmpty()
          ? Future.succeededFuture()
          : queryExecutor.execute(dslContext -> {
            InsertValuesStep3<FailedInstancesIdsRecord, UUID, UUID, UUID> insertValues =
                dslContext.insertInto(FAILED_INSTANCES_IDS, FAILED_INSTANCES_IDS.ID,
                  FAILED_INSTANCES_IDS.REQUEST_ID, FAILED_INSTANCES_IDS.INSTANCE_ID);
      ...
      var saveSkippedInstancesIds = holder.getSkippedInstancesIds().isEmpty()
          ? Future.succeededFuture()
          : queryExecutor.execute(dslContext -> {
            InsertValuesStep3<SkippedInstancesIdsRecord, UUID, UUID, UUID> insertValues =
                dslContext.insertInto(SKIPPED_INSTANCES_IDS, SKIPPED_INSTANCES_IDS.ID,
                  SKIPPED_INSTANCES_IDS.REQUEST_ID, SKIPPED_INSTANCES_IDS.INSTANCE_ID);
      ...
      var saveSuppressedFromDiscoveryInstancesIds = holder.getSuppressedInstancesIds().isEmpty()
          ? Future.succeededFuture()
          : queryExecutor.execute(dslContext -> {
            InsertValuesStep3<SuppressedFromDiscoveryInstancesIdsRecord, UUID, UUID, UUID>
                insertValues = dslContext.insertInto(SUPPRESSED_FROM_DISCOVERY_INSTANCES_IDS,
  ...
  private Record toDatabaseRecord(RequestMetadataLb requestMetadata) {
    return new RequestMetadataLbRecord().setRequestId(requestMetadata.getRequestId())
        ...
        .setLinkToErrorFile(requestMetadata.getLinkToErrorFile())
  ...
  public Future<Void> saveInstances(List<Instances> instances, String tenantId) {
    ...
    return getQueryExecutor(tenantId)
      .transaction(queryExecutor ->
          queryExecutor.execute(dslContext -> {
            InsertValuesStep3<InstancesRecord, UUID, UUID, Boolean> insertValues =
                dslContext.insertInto(INSTANCES, INSTANCES.INSTANCE_ID,
                INSTANCES.REQUEST_ID, INSTANCES.SUPPRESS_FROM_DISCOVERY);
            instances.forEach(instance ->
                insertValues.values(instance.getInstanceId(), instance.getRequestId(),
  ...
  public Future<List<String>> getRequestMetadataIdsByStartedDateAndExistsByPathToErrorFileInS3(
    ...
    return getQueryExecutorReader(tenantId).transaction(queryExecutor -> queryExecutor
        ...
        .map(queryResult ->
            queryResultToRequestMetadataListWithProperStartedDate(queryResult, date));
  }

  private List<String> queryResultToRequestMetadataListWithProperStartedDate(
      QueryResult queryResult, OffsetDateTime date) {
    return queryResult.stream()
        .map(QueryResult::unwrap)
        .map(Row.class::cast)
        .filter(row -> row.getOffsetDateTime(REQUEST_METADATA_LB.STARTED_DATE.getName())
            .isBefore(date))
  ...
  private List<Instances> queryResultToInstancesList(QueryResult queryResult) {
    return queryResult.stream()
        ...
        .map(row -> {
          ...
          pojo.setSuppressFromDiscovery(row.getBoolean(
              INSTANCES.SUPPRESS_FROM_DISCOVERY.getName()));
  ...
  private List<String> mapRequestIdsResultToList(QueryResult requestIds) {
    List<String> ids = requestIds.stream()
        .map(row -> {
          RequestMetadataLb pojo = RowMappers.getRequestMetadataLbMapper().apply(row.unwrap());
  ...
  private RequestMetadataCollection queryResultToRequestMetadataCollection(
      QueryResult queryResult, int totalRecordsCount) {
  ...
  private UuidCollection queryResultToUuidCollection(QueryResult queryResult,
  ...
  private RequestMetadata rowToRequestMetadata(Row row) {
    var pojo = RowMappers.getRequestMetadataLbMapper().apply(row);
