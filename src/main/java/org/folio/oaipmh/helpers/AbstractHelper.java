...
public abstract class AbstractHelper implements VerbHelper {
  ...
  public static Response buildNoRecordsFoundOaiResponse(OAIPMH oaipmh, Request request,
    ...
    if (StringUtils.isNotEmpty(errorMessage)) {
      ...
      if (verb == VerbType.GET_RECORD) {
        oaipmh.withErrors(new OAIPMHerrorType().withCode(ID_DOES_NOT_EXIST)
            .withValue(errorMessage));
      } else {
        oaipmh.withErrors(new OAIPMHerrorType().withCode(NO_RECORDS_MATCH)
            .withValue(errorMessage));
      }
    } else {
      oaipmh.withErrors(createNoRecordsFoundError());
  ...
  protected static OAIPMHerrorType createInvalidJsonContentError() {
    return new OAIPMHerrorType().withCode(INVALID_RECORD_CONTENT)
        .withValue(INVALID_CHARACTER_IN_THE_RECORD);
  }
   ...
   * @return {@link List} of the {@link OAIPMHerrorType} if there is any validation error
   *         or empty list
   */
  ...
  private void validateDateRange(Request request, List<OAIPMHerrorType> errors) {
    ...
    if (request.getFrom() != null) {
      ...
      if (from == null) {
        // In case the 'from' date is invalid, it cannot be sent in OAI-PMH/request@from because
        // it contradicts schema definition
    ...
    if (request.getUntil() != null) {
      ...
      if (until == null) {
        // In case the 'until' date is invalid, it cannot be sent in OAI-PMH/request@until
        // because it contradicts schema definition
  ...
  /**
   * Validates if the date time is in
   * {@linkplain org.folio.oaipmh.Constants#ISO_UTC_DATE_TIME ISO_UTC_DATE_TIME} format.
   * If the date is valid, {@link LocalDateTime} is returned. Otherwise
   * {@link #parseDate(Pair, List)} is called because repository must support
   * {@linkplain GranularityType#YYYY_MM_DD YYYY_MM_DD} granularity and date might be
   * in such format.
   *
   * @param date   {@link Pair} where key (left element) is parameter name (i.e. from or until),
   *                            value (right element) is date time
   ...
   */
  private Pair<GranularityType, LocalDateTime> parseDateTime(Pair<String, String> date,
   ...
   *
   * @param date   {@link Pair} where key (left element) is parameter name (i.e. from or until),
   *                            value (right element) is date
   * @param errors list of errors to be updated if format is wrong
   * @return {@link LocalDateTime} if date format is valid, {@literal null} otherwise.
   */
  private Pair<GranularityType, LocalDateTime> parseDate(Pair<String, String> date,
      List<OAIPMHerrorType> errors) {
    if (date.getValue().isEmpty()) {
      return null;
    }
  ...
  /**
   * Parse a date from string and compensate one date or one second because
   * in SRS the dates are non-inclusive.
   *
   * @param dateTimeString                 - date/time supplied
   * @param shouldCompensateUntilDate      = if the date is used as until parameter
   * @param shouldEqualizeTimeBetweenZones whether the time should be updated by
   *                                       diff between current time zone and UTC
  ...
  protected Date convertStringToDate(String dateTimeString, boolean shouldCompensateUntilDate,
      boolean shouldEqualizeTimeBetweenZones) {
    try {
  ...
  /**
   * Adds difference between the current time zone and UTC to 'date' param. Is used when 'date'
   * is going to be used for SRC client search criteria by the reason that SRC equates the
   * date to UTC date by subtracting the diff between current time zone and UTC
  ...
  protected ResumptionTokenType buildResumptionToken(Request request, JsonArray instances,
    ...
    if (newOffset < (request.isFromInventory()
        ? request.getInventoryTotalRecords() : totalRecords)) {
      ...
      extraParams.put(REQUEST_INVENTORY_TOTAL_RECORDS_PARAM,
          String.valueOf(request.getInventoryTotalRecords()));
      extraParams.put(REQUEST_INVENTORY_OFFSET_SHIFT_PARAM,
          String.valueOf(request.getInventoryOffsetShift()));
      extraParams.put(REQUEST_OLD_SRS_OFFSET_PARAM,
          String.valueOf(request.getOldSrsOffset()));
  ...
  /**
   * Returns string representation of built LocalDateTime object. If
   * repository.timeGranularity == yyyy-MM-dd format then only this
   * format has to be used for building LocalDateTime. If repository.timeGranularity ==
   * yyyy-MM-ddTHH:mm:ssZ then since such format allows both formats for 'from' parameter
   * (date+time or date only) then 'until' value should be built using the same format as
  ...
  protected String getUntilDate(Request request, String from) {
    ...
    if (isDateOnly) {
      ...
      return isNotEmpty(from) && from.matches(DATE_ONLY_PATTERN)
          ? LocalDateTime.now().format(ISO_UTC_DATE_ONLY)
          : "";
    }
  }

  /**
   * Filter records from source-record-storage by the fields "deleted = true" or MARC
   * Leader 05 is "d". For old behavior (repository.deletedRecords is "no") fields are
   * checked only according to the previously described criteria. In case when
   * repository.deletedRecords is "persistent" or "transient" checked suppressedRecordsProcessing
   * config setting.
   *
   * @param request  OIA-PMH request
  ...
  /**
   * Checks if request sequences can be resumed without losing records in case of
   * partitioning the whole result set.
   *
   * @param request      oai-pmh request
   ...
   */
  protected boolean canResumeRequestSequence(Request request, Integer totalRecords,
      JsonArray instances) {
  ...
  protected Future<Response> buildResponseWithErrors(Request request, Promise<Response> promise,
      List<OAIPMHerrorType> errors) {
    OAIPMH oai;
   ...
   * @return FOLIO if records source is 'Inventory records source', MARC if records source
   *         is 'Source record storage', null if records source is 'SRS + Inventory'
   */
        ...
        .onComplete(requestMetadataLbAsyncResult -> {
          if (requestMetadataLbAsyncResult.succeeded()) {
            ...
            if (!isBlank(linkToErrorFile)) {
              logger.info("Errors saved successfully for requestId {}",
