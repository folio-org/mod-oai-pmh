...
public class ItemsHoldingsEnrichment {
  ...
  public JsonParser getJsonParser() {
    ...
    jsonParser.handler(event -> {
      ...
      if (instance != null) {
        ...
        logger.info(
            "enrichInstances:: For requestId {} instance with instanceId {} wasn't in the request",
            request.getRequestId(), instanceId);
      }
    });
    jsonParser.exceptionHandler(throwable ->
        logger.error("enrichInstances:: For requestId {} error has been occurred at JsonParser"
            + " for items-and-holdings response, errors {}", request.getRequestId(),
  ...
  private void addEffectiveLocationCallNumberFromHoldingsWithoutItems(JsonObject instance) {
    ...
    for (Object holding : holdingsJson) {
      if (holding instanceof JsonObject) {
        ...
        if (excludeHoldingsIds.contains(holdingJson.getString(ID))) {
          continue;
        }
        ...
        itemJson.put(RecordMetadataManager.INVENTORY_SUPPRESS_DISCOVERY_FIELD,
            Boolean.valueOf(holdingJson.getString(
                RecordMetadataManager.INVENTORY_SUPPRESS_DISCOVERY_FIELD)));
  ...
  private void updateItems(JsonObject instance) {
    JsonArray itemsJson = instance.getJsonObject(ITEMS_AND_HOLDINGS_FIELDS).getJsonArray(ITEMS);
    for (Object item : itemsJson) {
      JsonObject itemJson = (JsonObject) item;
      itemJson.getJsonObject(LOCATION).put(NAME, itemJson.getJsonObject(LOCATION)
