...
public class ItemsHoldingsRequestWithDelayExecutor {
  ...
  private static final Logger logger =
      LogManager.getLogger(ItemsHoldingsRequestWithDelayExecutor.class);
  ...
  public void execute(long delay, String instanceId,
      Promise<List<JsonObject>> enrichInstancesPromise, ErrorsService errorsService) {
    vertx.setTimer(delay, id -> {
      var httpRequest = ItemsHoldingInventoryRequestFactory
          .getItemsHoldingsInventoryRequest(itemsHoldingsEnrichment.getRequest());
      JsonObject entries = new JsonObject();
      entries.put(INSTANCE_IDS_ENRICH_PARAM_NAME, new JsonArray(List.of(instanceId)));
      entries.put(SKIP_SUPPRESSED_FROM_DISCOVERY_RECORDS,
          itemsHoldingsEnrichment.isSkipSuppressed());
        ...
        .onSuccess(response -> {
          if (response.statusCode() != 200) {
            ...
            errors.forEach(error -> {
              errorsService.log(itemsHoldingsEnrichment.getRequest().getTenant(),
          ...
          if (inc.get() < size) {
            ...
            enrichInstancesPromise.complete(
                new ArrayList<>(itemsHoldingsEnrichment.getInstancesMap().values()));
          }
