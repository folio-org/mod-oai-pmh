...
public class GetListRecordsRequestHelper extends AbstractGetRecordsHelper {
  ...
  public Future<Response> handle(Request request, Context vertxContext) {
    ...
    try {
      ...
      processBatch(request, oaipmhResponsePromise, isFirstBatch, batchInstancesStatistics,
          lastUpdateDate);
    ...
    return oaipmhResponsePromise.future().onComplete(responseAsyncResult -> {
      logger.info("Total time for response: {} sec",
          (System.nanoTime() - t) / 1_000_000_000);
  ...
  private void processBatch(Request request, Promise<Response> oaiPmhResponsePromise,
    ...
    try {
      ...
      int retryAttempts = Integer
          .parseInt(getProperty(request.getRequestId(),
              REPOSITORY_SRS_HTTP_REQUEST_RETRY_ATTEMPTS));
      ...
      requestRecords(request, retryAttempts, supportCompletedSize, statistics)
          .onSuccess(records -> {
            if (records.isEmpty() && !firstBatch) {
              logger.error(
                  "processBatch:: For requestId {} instances collection is empty "
                  + "for non-first batch", request.getRequestId());
              oaiPmhResponsePromise.complete(buildBadResumptionTokenOaiResponse(oaipmhResponse,
                  request, "Specified resumption token doesn't exists."));
              return;
            }
              var listRecords = records.stream().map(JsonObject.class::cast).toList();
              long t = System.nanoTime();
              enrichInstances(listRecords, request)
                  .onComplete(listAsyncResult -> {
                    logger.info("After enrich: {} sec",
                        (System.nanoTime() - t) / 1_000_000_000);
                    if (listAsyncResult.succeeded()) {
                      buildRecordsResponse(request, lastUpdateDate,
                          new JsonArray(listAsyncResult.result()), firstBatch, statistics)
                          .onSuccess(oaiPmhResponsePromise::complete)
                          .onFailure(e ->
                              oaiPmhResponsePromise.complete(buildNoRecordsFoundOaiResponse(
                                  oaipmhResponse, request, e.getMessage())));
                      metricsCollectingService.endMetric(request.getRequestId(),
                          INSTANCES_PROCESSING);
                    } else {
                      logger.error("Request records failed : {}",
                          listAsyncResult.cause().getMessage(), listAsyncResult.cause());
                    }
                  });
            }
          )
          .onFailure(e -> oaiPmhResponsePromise.complete(buildNoRecordsFoundOaiResponse(
              oaipmhResponse, request, e.getMessage())));
    } catch (Exception e) {
      handleException(oaiPmhResponsePromise, e);
    }
  ...
  private Response buildBadResumptionTokenOaiResponse(OAIPMH oaipmh, Request request,
      String message) {
  ...
  private Future<JsonArray> requestRecords(Request request, int retryAttempts,
      boolean supportCompletedSize, StatisticsHolder statistics) {
    int limit = Integer.parseInt(getProperty(request.getRequestId(),
        REPOSITORY_MAX_RECORDS_PER_RESPONSE)) + 1;
    ...
    doRequest(request, finalRecords, skipSuppressedFromDiscovery, deletedRecordsSupport, from,
        until, RecordsSource.getSource(recordsSource), limit, limit, supportCompletedSize,
        statistics)
        .onComplete(handler -> {
          if (handler.succeeded()) {
            ...
            if (!idJsonMap.isEmpty()) {
              ...
              if (StringUtils.isNotEmpty(centralTenantId)) {
                doRequestShared(vertx, deletedRecordsSupport, idJsonMap, attemptsCount,
                    retryAttempts, shared, new Request(request, centralTenantId), finalRecords);
              } else {
            ...
            logger.error("Request records was not succeeded: {}",
  ...
  private void excludeSharedMarcRecords(JsonArray finalRecords,
      Map<String, JsonObject> idJsonMap) {
  ...
  private void doRequestShared(Vertx vertx, boolean deletedRecordsSupport,
    ...
    logger.info("doRequestShared execution: {}, {}, {}",
        idJsonMap.keySet(), deletedRecordsSupport, request);
    var client =
    ...
    client.postSourceStorageSourceRecords("INSTANCE",
        ...
        if (asyncResult.succeeded()) {
          ...
          if (statusCode >= 400) {
            ...
            retrySrsRequest(vertx, deletedRecordsSupport, idJsonMap, attemptsCount, promise,
                retrySrsRequestParams, request, records);
          }
          if (statusCode != 200) {
            String errorMsg = getErrorFromStorageMessage("source-record-storage",
                "/source-storage/source-records", srsResponse.statusMessage());
          ...
          logger.error("Error has been occurred while requesting the SRS: {}.",
  ...
  private void retrySrsRequest(Vertx vertx, boolean deletedRecordsSupport,
      Map<String, JsonObject> idJsonMap, AtomicInteger attemptsCount, Promise<JsonArray> promise,
      Map<String, String> retrySrsRequestParams, Request request, JsonArray records) {
    if (Integer.parseInt(retrySrsRequestParams.get(STATUS_CODE)) > 0) {
      logger.debug("Got error response form SRS, status code: {}, status message: {}.",
          Integer.parseInt(retrySrsRequestParams.get(STATUS_CODE)),
          retrySrsRequestParams.get(STATUS_MESSAGE));
    } else {
      logger.debug("Error has been occurred while requesting SRS.");
    }
    if (attemptsCount.decrementAndGet() <= 0) {
      String errorMessage =
          "mod-source-record-storage didn't respond with expected status code after "
          + Integer.parseInt(retrySrsRequestParams.get(RETRY_ATTEMPTS))
  ...
  private Future<Void> doRequest(Request request, JsonArray finalRecords,
      ...
      String until, RecordsSource source, int limit, int currentLimit,
      boolean supportCompletedSize, StatisticsHolder statistics) {
    try {
      var query = QueryBuilder.build(request.getTenant(), request.getLastInstanceId(),
          from, until, source, skipSuppressedFromDiscovery,
          request.isFromDeleted() && deletedRecordsSupport, currentLimit);
      logger.info("Query: {}", query);
      return viewsService.query(query, request.getTenant())
          .compose(currentRecords ->
              handleCompleteListSize(request, supportCompletedSize, currentRecords))
          .onFailure(completeListSizeHandlerExc ->
              logger.error("Error occurred while calling a view: {}.",
              completeListSizeHandlerExc.getMessage(), completeListSizeHandlerExc))
          .compose(currentRecords ->
              processInstancesFromDb(request, currentRecords, finalRecords,
                  skipSuppressedFromDiscovery, deletedRecordsSupport, from, until, source,
                  supportCompletedSize, statistics, limit))
          .onFailure(collectDeletedHandlerExc ->
              logger.error("Error occurred while collecting deleted: {}.",
  ...
  private Future<Void> processInstancesFromDb(Request request, JsonArray currentRecords,
      JsonArray finalRecords, boolean skipSuppressedFromDiscovery, boolean deletedRecordsSupport,
      String from, String until, RecordsSource source, boolean supportCompletedSize,
      StatisticsHolder statistics, int limit) {
    finalRecords.addAll(currentRecords);

    logger.info("Number of records before handleDeletedRecords found: {}",
        currentRecords.size());
  ...
  private Future<Void> handleDeletedRecords(Request request, JsonArray finalRecords,
      boolean skipSuppressedFromDiscovery, boolean deletedRecordsSupport, String from,
      String until, RecordsSource source, boolean supportCompletedSize,
      StatisticsHolder statistics, int limit) {
    ...
    if (deletedRecordsSupport && !request.isFromDeleted()) {
      ...
      if (remainingFromDeleted > 0 || finalRecords.isEmpty()) {
        // Last instance id should be null when switching to deleted.
  ...
  private void updateRequest(Request request, JsonArray finalRecords, int limit,
      StatisticsHolder statistics) {
    if (finalRecords.size() > 1 && finalRecords.size() >= limit) {
      request.setNextRecordId(
          finalRecords.getJsonObject(finalRecords.size() - 1)
              .getString(INSTANCE_ID_FROM_VIEW_RESPONSE));
      if (!request.isTurnedToDeleted()) {
        request.setLastInstanceId(
            finalRecords.getJsonObject(finalRecords.size() - 2)
                .getString(INSTANCE_ID_FROM_VIEW_RESPONSE));
      }
    } else {
    ...
    logger.debug("Execution of handleMARCsWithoutSRSRecord: {} sec",
        (System.nanoTime() - t) / 1_000_000_000);
  ...
  private JsonArray handleMarcsWithoutSrsRecord(Request request, JsonArray currentRecords,
      StatisticsHolder statistics) {
    return new JsonArray(currentRecords.stream().map(JsonObject.class::cast)
      .filter(rec -> {
        ...
        if (!rec.getString(SOURCE).equals(RecordsSource.MARC.name())
            && !rec.getString(SOURCE).equals(RecordsSource.LINKED_DATA.name())
        ...
        if (isNull(marcRecord)) { // If MARC does not have underlying SRS record.
          ...
          errorsService.log(request.getTenant(), request.getRequestId(),
              rec.getString(INSTANCE_ID_FROM_VIEW_RESPONSE),
  ...
  private Future<JsonArray> handleCompleteListSize(Request request, boolean supportCompletedSize,
      JsonArray currentRecords) {
    if (supportCompletedSize && request.getCompleteListSize() == 0) {
      return requestFromInventory(request, 1, null, false, false, false)
          .compose(counts -> {
          ...
          .onFailure(handleException ->
              logger.error("Error occurred while calling /inventory-storage/instances "
                + "to get complete list size: {}.",
    ...
    try {
      ...
      if (oaipmh.getErrors().isEmpty()) {
        if (!firstBatch || nextInstanceId != null) {
          ResumptionTokenType resumptionToken = buildResumptionTokenFromRequest(request,
              request.getRequestId(), records.size(), nextInstanceId);
          if (request.getVerb() == VerbType.LIST_IDENTIFIERS) {
      ...
      instancesService.updateRequestUpdatedDateAndStatistics(request.getRequestId(),
          lastUpdateDate, statistics, request.getTenant())
          .onComplete(x -> {
            logger.debug("Execution of buildRecordsResponse: {} sec",
                (System.nanoTime() - t) / 1_000_000_000);
  ...
  private List<RecordType> buildRecordsList(Request request, JsonArray srsResponse,
      StatisticsHolder statistics) {
    ...
    srsResponse.stream()
        ...
        .forEach(instance -> {
          ...
          JsonObject updatedSrsWithItemsData = metadataManager.populateMetadataWithItemsData(
              instance, instance, suppressedRecordsProcessing);
          JsonObject updatedSrsRecord = metadataManager.populateMetadataWithHoldingsData(
              updatedSrsWithItemsData, instance, suppressedRecordsProcessing);
          ...
          if (source != null && instanceRecord.getHeader().getStatus() == null) {
            if (suppressedRecordsProcessing) {
              source = metadataManager.updateMetadataSourceWithDiscoverySuppressedData(
                  source, updatedSrsRecord);
              source = metadataManager.updateElectronicAccessFieldWithDiscoverySuppressedData(
                  source, updatedSrsRecord);
            }
            try {
  ...
  private ResumptionTokenType buildResumptionTokenFromRequest(Request request, String requestId,
        long returnedCount, String nextInstanceId) {
    ...
    extraParams.put(EXPIRATION_DATE_RESUMPTION_TOKEN_PARAM,
        String.valueOf(Instant.now().with(ChronoField.NANO_OF_SECOND, 0)
            .plusSeconds(RESUMPTION_TOKEN_TIMEOUT)));
    ...
    var resumptionTokenType = new ResumptionTokenType()
        .withExpirationDate(Instant.now().with(ChronoField.NANO_OF_SECOND, 0)
            .plusSeconds(RESUMPTION_TOKEN_TIMEOUT))
        .withCursor(BigInteger.valueOf(cursor));
    String targetMetadataPrefix = request.getMetadataPrefix();
    if ((request.getVerb().equals(VerbType.LIST_IDENTIFIERS)
      ...
      extraParams.put(REQUEST_COMPLETE_LIST_SIZE_PARAM,
          String.valueOf(request.getCompleteListSize()));
    }
    String resumptionToken = request.toResumptionToken(extraParams);
    resumptionTokenType.withValue(resumptionToken);
    long t = System.nanoTime();
    logger.debug("Execution of buildResumptionTokenFromRequest: {} sec",
        (System.nanoTime() - t) / 1_000_000_000);
    return resumptionTokenType;
  }

  private void logHarvestingCompletion() {
    logger.info("Harvesting completed. Number of processed instances: {}.",
        batchesSizeCounter.get());
    batchesSizeCounter.setRelease(0);
  }

  private void updateWithParsedRecord(JsonArray records, Promise<JsonArray> promise,
      Request request) {
